...
volatile static systemTime_t millis;
...
struct time_t scheduler_getTimeFormated() {
	struct time_t time;
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
		time.milli = (uint16_t)millis % MILLI_PER_SEC;
		time.second = (uint8_t)(millis / MILLI_PER_SEC) % SEC_PER_MIN;
		time.minute = (uint8_t)(millis / MILLI_PER_SEC / SEC_PER_MIN) % MIN_PER_HOUR;
		time.hour = (uint8_t)(millis / MILLI_PER_SEC / SEC_PER_MIN / MIN_PER_HOUR) % HOURS_PER_DAY;
	}
	return time;
}
