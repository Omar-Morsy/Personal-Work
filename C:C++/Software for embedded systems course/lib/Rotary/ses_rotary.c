...
void rotary_init() {
	...
	DDR_REGISTER(ROTARY_PORTA) &= (0 << ROTARY_PINA);
	DDR_REGISTER(ROTARY_PORTB) &= (0 << ROTARY_PINB);
...
void rotary_checkState() {
    ...
    // we read ROTARY_NUM_DEBOUNCE_CHECKS consistent "1" in the state
    // array, the rotary at this position is considered pressed
    uint8_t j = 0xFF;
    ...
    //check if rotary in clockwise (rotary debouncing)
    if (((debouncedState & 1) == 1) && (rotary_clockwiseCallback != NULL) && (debouncedState != lastDebouncedState)) {
    	rotary_clockwiseCallback();
    }

    //check if rotary in counter-clockwise (rotary debouncing)
    if ((((debouncedState>>1) & 1) == 1) && (rotary_counterClockwiseCallback != NULL) && (debouncedState != lastDebouncedState)) {
